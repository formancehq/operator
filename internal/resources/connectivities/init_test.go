...
// unstructuredStackIndex indexes both the typed objects returned by the current
// dependency cache and the unstructured objects used by delegated APIs.
func unstructuredStackIndex(object client.Object) []string {
	...
	if value, found, err := unstructured.NestedString(unstructuredObject.Object, "spec", "stack"); err == nil && found {
		return []string{value}
	}
	return nil
}
...
// The unsatisfied-requirements handler must surface a gate lookup failure so
...
func TestHandleUnsatisfiedLedgerRequirementSurfacesGateLookupFailure(t *testing.T) {
	...
	if err := handleUnsatisfiedLedgerRequirement(ctx, stack, connectivity); err == nil {
		t.Fatal("handleUnsatisfiedLedgerRequirement must surface the gate lookup failure for retry")
	}
}
