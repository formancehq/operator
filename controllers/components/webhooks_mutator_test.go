		...
		WithNewNamespace(func() {
			Context("When creating a webhooks server", func() {
						...
						Spec: componentsv1beta2.WebhooksSpec{
							Collector: &componentsv1beta2.CollectorConfig{
								KafkaConfig: apisv1beta2.KafkaConfig{
									...
									TLS:     false,
									SASL: &apisv1beta2.KafkaSASLConfig{
										Username:     "username",
										Password:     "password",
										Mechanism:    "mechanism",
				...
				It("Should create a deployment", func() {
					Eventually(ConditionStatus(webhooks, apisv1beta2.ConditionTypeDeploymentReady)).Should(Equal(metav1.ConditionTrue))
				...
				It("Should create a service", func() {
					Eventually(ConditionStatus(webhooks, apisv1beta2.ConditionTypeServiceReady)).Should(Equal(metav1.ConditionTrue))
				...
				Context("Then enable ingress", func() {
					...
					It("Should create a ingress", func() {
						Eventually(ConditionStatus(webhooks, apisv1beta2.ConditionTypeIngressReady)).Should(Equal(metav1.ConditionTrue))
					...
					Context("Then disabling ingress support", func() {
						BeforeEach(func() {
							Eventually(ConditionStatus(webhooks, apisv1beta2.ConditionTypeIngressReady)).
