...
var _ = Describe("Test Search Ingester", func() {
	...
	BeforeEach(func() {
		...
		Expect(nsClient.Create(ctx, search)).To(Succeed())
	})
	AfterEach(func() {
		Expect(nsClient.Delete(ctx, search)).Should(Succeed())
	})
	Context("With a search deployed, when creating a new SearchIngester object", func() {
		...
		BeforeEach(func() {
			...
			Expect(nsClient.Create(ctx, searchIngester)).To(Succeed())
			Eventually(ConditionStatus(nsClient, searchIngester, ConditionTypeReady)).
		...
		It("Should create a BenthosStream object", func() {
			...
			Eventually(Exists(nsClient, stream)).Should(BeTrue())
		})
	})
})
